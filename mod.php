...
class Router
{
    ...
    private function initializePages(): void
    {
        $this->pages = [
            ...
            '/(\%b)/' . str_replace('%d', '(\d+)', preg_quote($this->config['file_page'], '!')) => 'view_board',
            ...
            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace('%d', '(\d+)', preg_quote($this->config['file_page'], '!')) => 'view_thread',
            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace(array('%d', '\.html'), array('(\d+)', ''), preg_quote($this->config['file_page'], '!')) => 'view_thread',

            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace('%d', '(\d+)', preg_quote($this->config['file_page50'], '!')) => 'view_thread50',
            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace(array('%d', '\.html'), array('(\d+)', ''), preg_quote($this->config['file_page50'], '!')) => 'view_thread50',

            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace(array('%d', '%s'), array('(\d+)', '[a-z0-9-]+'), preg_quote($this->config['file_page50_slug'], '!')) => 'view_thread50',
            '/(\%b)/' . preg_quote($this->config['dir']['res'], '!') .
                str_replace(array('%d', '%s'), array('(\d+)', '[a-z0-9-]+'), preg_quote($this->config['file_page_slug'], '!')) => 'view_thread',
        ];
