...
if (file_exists($config['has_installed'])) {
	...
	switch ($version) {
		...
		default:
			...
			$page['body'] = '<p style="text-align:center">nicochan was unable to determine what version is currently installed.</p>';
			break;
		case VERSION:
			$page['title'] = 'Already installed';
			$page['body'] = '<p style="text-align:center">It appears that nicochan is already installed (' . $version . ') and there is nothing to upgrade! Delete <strong>' . $config['has_installed'] . '</strong> to reinstall.</p>';
			break;
	}
...
if ($step == 0) {
	...
	$tests = [
		...
		[
			...
			'required' => false,
			'message' => 'It is highly recommended that you install the PHP <a href="http://www.php.net/manual/en/openssl.installation.php">OpenSSL</a> extension. Installing the OpenSSL extension allows Nicochan to generate a secure salt automatically for you.',
		],
		[
			'category' => 'Database',
		...
		[
			...
			'prereq' => '',
			'result' => $can_exec && shell_exec('echo "nicochan" | md5sum') == "fcbcbd327f6d4a109d5fc00627e34954  -\n",
			'required' => false,
		...
		[
			...
			'result' => $can_exec && shell_exec('echo "nicochan" | /sbin/md5 -r') == "fcbcbd327f6d4a109d5fc00627e34954\n",
			'required' => false,
		...
		[
			...
			'result' => is_writable('.'),
			'required' => true,
			'message' => 'Nicochan does not have permission to create directories (boards) here. You will need to <code>chmod</code> (or operating system equivalent) appropriately.'
		],
		[
			'category' => 'File permissions',
		...
		[
			...
			'required' => false,
			'message' => 'Nicochan does not have permission to make changes to <code>inc/instance-config.php</code>. To complete the installation, you will be asked to manually copy and paste code into the file instead.'
		],
		[
			'category' => 'Misc',
		...
		[
			'category' => 'Misc',
			'name' => 'Nicochan installed using git',
			'result' => is_dir('.git'),
			'required' => false,
			'message' => 'Nicochan is still beta software and it\'s not going to come out of beta any time soon. As there are often many months between releases yet changes and bug fixes are very frequent, it\'s recommended to use the git repository to maintain your Nicochan installation. Using git makes upgrading much easier.'
		],
		[
			'category' => 'Misc',
...
';
	...
	$page['body'] = '<p style="text-align:center">Thank you for using Nicochan. Please remember to report any bugs you discover. <a href="https://github.com/fallenPineapple/NPFchan/wiki/Configuration">How do I edit the config files?</a></p>';
	...
	$page['title'] = 'Installation complete';
	$page['body'] = '<p style="text-align:center">Thank you for using Nicochan. Please remember to report any bugs you discover.</p>';
